...
; visible -- see RS_PENDING for the marker and the cap on waiting.
;
; State image (769 bytes, positions 0..768) -- the family's proven layout,
; UNCHANGED in size (ram.asm explains why the tail's SC_CNT2-SC_CNT7
; repurposing needed no sweep):
;   0..146    $015D-$01EF game scratch (bytes; the whole game state --
;             scores, round/match phase, AND the SAVED EXEC ISR VECTOR at
;             $01A8/$01A9.  That pair is a code pointer the cart's ISR body
;             jumps through unconditionally when $0102 falls out of range,
;             so RS_REBASE clamps it to the EXEC default -- a corrupted
;             wire byte there would install an arbitrary interrupt handler,
;             past every bounds check, because the POSITION is legal.
;             PORTING.md §7.27's class.)
;   147..626  $0200-$02EF BACKTAB (240 words, LE byte pairs)
;   627..754  $031D-$035C object table (64 words, LE byte pairs)
;   755..768  RNG_LO/HI, two spares, SC_CNT2..SC_CNT7 (the block
;             $8196-$819B -- THREE real countdown mirrors on this cart, not
;             one; ram.asm), one spare, SC_PASSLEN ($0103 -- not written by
...
; sweep of every numeric bound in the applier and pusher.  Not needed here:
; the tail's total size is IDENTICAL to every prior 2-seat port's, only
; which cells inside it are "real" changed.)
...
@@rc_bad:
        ...
        ; instant a CRC disagrees lands the state image mid-round; RS_PENDING
        ; holds the push until BX_QUIESCENT.  The guest simply keeps playing
        ; until the host's BEGIN arrives -- it must NOT freeze here, or it
        ; would sit still through the very round we are waiting to finish.
        MVII    #1,     R0
        MVO     R0,     RS_PEND
        CLRR    R0
        MVO     R0,     RS_PTMO
...
@@rb_hd:
    ENDI
        ...
        JSR     R5,     RS_CLAMP_ISR
        JSR     R5,     BX_REBASE_HOOK   ; cart-specific (src/hook.asm):
                                        ;  restores the three native
...
;
; BX_QUIESCENT (ram.asm/exec_equ.asm/src/hook.asm) is derived every tick
; from BX_MATCHOVER ($017C).  Waiting costs a few seconds of the two sims
; running visibly apart, which is why the wait is capped at RS_PEND_MAX
; ticks -- past that a visible jump beats staying desynced, and we push
; regardless.
;
...
; ---------------------------------------------------------------------------
RS_PEND_MAX     EQU     60              ; game ticks (~3 s at the measured
                                        ;  tick -- confirm at M2)
...
RS_PENDING:
        ...
        INCR    R0
        MVO     R0,     RS_PTMO
        ; quiescent?  ask the cart's own "match is over" predicate.
        MVI     SC_PHASE, R0
        ANDI    #SC_PHASE_DEAD, R0
        BNEQ    @@rp_go                 ; quiescent: the swap is invisible
@@rp_tmo:
        MVI     RS_PTMO, R0
        CMPI    #RS_PEND_MAX, R0
        BLT     @@rp_out                ; still waiting for a quiet moment
        MVII    #2,     R0              ; gave up: pushing mid-round
        B       @@rp_mark
@@rp_go:
...
; ---------------------------------------------------------------------------
; RS_DISPLAY_RESET -- reassert the static display state from the cart
; header.  Boxing is a COLOUR-STACK cart that never writes the STIC or GRAM
; from game logic beyond its own boot-time init and the per-frame ISR
; strobe of $0020 (display enable) -- so the header words are genuinely
; static and reasserting them is a no-op when healthy (Baseball's original
; form, not Auto Racing/Soccer's scroll-aware one: Boxing has no scroll
; registers to avoid fighting).
; ---------------------------------------------------------------------------
RS_DISPLAY_RESET:
        PSHR    R5
        MVI     $500D,  R0
...
;
; $01A8/$01A9 is where Boxing's .START stashes $0100/$0101 before installing
; its own ISR body, and the body jumps through that pair unconditionally
; whenever $0102 falls out of range.  It sits inside $015D-$01EF, so it
; rides in image section 1 whether we want it to or not.  In a healthy
; machine it is always $1126 and this routine is a no-op -- but a single
; corrupted wire byte landing there would install an ARBITRARY ADDRESS as
; the frame interrupt handler on the next out-of-range tick and brick the
; console.  None of the existing image guards can catch it: they bounds-
; check the POSITION, and this position is legal.  PORTING.md §7.27's
; code-pointer clamp.
;
...
; 0), compared against nothing and declared a desync.  CRCs are only ever
; recorded on a 64-tick boundary, so a stored tick_lo of $FF cannot collide
; with a real one.
RS_CLR_CRC:
        PSHR    R5
        MVII    #OWN_CRC, R4
...
; ---------------------------------------------------------------------------
; 14 entries, the family's proven span.  SC_CNT2-SC_CNT7 are all REAL here
; (three native countdown mirrors, ram.asm) where every prior 2-seat port
; had at most one real pair plus spares -- the total entry count and every
; IMG_S3/IMG_TOTAL bound stay unchanged; only which cells are "real" moved.
RS_TAILTBL:
        DECLE   RNG_LO, RNG_HI
        DECLE   RS_SPARE0, RS_SPARE1
        DECLE   SC_CNT2, SC_CNT3, SC_CNT4          ; $8196-$8198
        DECLE   SC_CNT5, SC_CNT6, SC_CNT7          ; $8199-$819B
