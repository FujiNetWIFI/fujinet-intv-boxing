...
@@ls_hd:
        PULR    R7
@@ls_go0:
        ; Every pass is a game tick on this cart (fast entries interval 1).
        ...
        ADDR    R1,     R2              ; R2 = T+d
    IF NET_FUZZ <> 0
        ; Rig builds: the demo script first (boot, CHOOSE MEN prompt,
        ; opening seconds of a round), then pseudo-random input seeded per
        ; console from boot entropy.  The script has two columns and this
        ; cart has exactly two seats, so seat s reads column s -- both
        ; columns are live every tick, no arbitration.
...
@@ls_run:
        ...
        EIS
@@ls_nf:
        ; Boxing is DISPATCH-ONLY (M0): no polled $011F/$0120 surface, so
        ; there is no UPDATE_SHADOW/SHADOW_FROM_RINGS call here at all --
        ; unlike every prior 2-seat port.  Handler-table swap,
        ; non-destructive: the game (and EXEC) install new tables from tick
        ; code AND from dispatched handlers, so adopt whatever is live
        ; before overwriting, run the tick + virtual dispatch with the REAL
        ; table installed (handlers may re-install mid-dispatch, stock-
        ; style), adopt again, then null it out so the real scan's dispatch
        ; stays inert until the next tick.
        JSR     R5,     LS_TBL_ADOPT
        MVI     GAME_TBL_HI, R1
        SWAP    R1,     1
        ADD     GAME_TBL_LO, R1
...
@@ls_no_tbl:
        ...
        ; shared per-seat handler body indexes state off R1 (the controller
        ; index, VD_CTRL) directly, and the two boxers act simultaneously,
        ; so suppressing either seat would drop half the input.
        CLRR    R0
        ...
        MVO     R0,     VD_CTRL
        JSR     R5,     LS_VDISPATCH
        JSR     R5,     BX_GAME_TICK    ; BX_TICK1 + native-countdown mirror
        JSR     R5,     BX_POSSESSION   ; stub -- no possession concept
        JSR     R5,     LS_TBL_ADOPT
        ...
        ANDI    #$3F,   R0
        BNEQ    @@ls_nocrc
        ; The static display state (colour stack, border) is written once
        ; at start-of-game and never refreshed, so anything that scribbles
...
;
; DANCE_SETTLE (vdispatch.asm, unchanged) waits out Boxing's PERMANENTLY
; installed custom ISR ($58E1) before repainting -- SC_ISR_BODY
; (exec_equ.asm) is set so the check is always true, so this always spins
; its full bound and force-restores the EXEC default ISR (PORTING.md §7.10:
; the cart's own body never voluntarily uninstalls itself, so DANCE_SETTLE
; must not wait for it to).
;
...
LS_PEER_LEFT:
        PSHR    R5
        JSR     R5,     DANCE_SETTLE    ; force-restore the EXEC default ISR
...
; $015D-$01EF game scratch + canonical RNG + the native timer countdown
; mirrors (all six real cells -- three timer-API-stoppable entries, one
; 16-bit mirror each).  Must match src/debug.asm TRACE_RANGES.
LS_CKSUM:
        PSHR    R5
        CLRR    R0
        MVII    #$15D,  R4
@@lk_1: SLLC    R0,     1
        ...
        SLLC    R0,     1
        ADCR    R0
        ; Native timer tail.  Only SC_CNT2-SC_CNT7 carry state on this cart
        ; (six real cells, all three native entries armed/stopped by the
        ; cart), but the spans stay exactly the family's so this tail,
        ; debug.asm's TRACE_RANGES and resync.asm's RS_TAILTBL agree
        ; without a §7.16 bounds sweep.  The RS_SPARE cells are always
        ; zero and contribute nothing.
        ADD     RS_SPARE0, R0
        SLLC    R0,     1
        ADCR    R0
        ADD     RS_SPARE1, R0
        MVII    #SC_CNT2, R4            ; $8196-$819B: all six real
@@lk_2: SLLC    R0,     1
        ADCR    R0
        ADD@    R4,     R0
        CMPI    #SC_CNT7+1, R4
