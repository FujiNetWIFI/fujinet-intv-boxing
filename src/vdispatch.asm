...
; Demo script for Boxing.  Column dec0/kp0 = seat 0, dec1/kp1 = seat 1 --
; both are live every tick (no turn arbiter, src/exec_equ.asm/hook.asm).
;
; The pre-game "CHOOSE MEN" prompt (GAME_TBL == BX_HTBL_PROMPT == $5932) is
; keypad-driven, not disc-driven, and masked ($3F) fuzz can NEVER answer it
; (PORTING.md §7.25) -- confirmed by decoding the prompt's own two live
; handlers (M0, dis1600): the disc slot (table+0, $5347) just tracks a
; preview cursor, and the keypad-digit slot (table+2, $597E) is where
; selection actually happens -- `CMPI #$0006,R0 / BGT ...reject...` accepts
; digits 1-6 (six boxers to choose from) and `CMPI #$000B,R0 / BEQ
; ...confirm...` is ENTER (R0=$0B, matching every sibling port's ARB_INJECT
; convention for the same key).  Digit/ENTER events reach LS_VDISPATCH
; through the SAME $011F-style decoded cell as disc (bit 7 set: $80|k for
; digit k, $8B for ENTER), dispatched to table+2 by the disc-scan's own
; keypad branch (@@vd_ds above) -- there is no separate SEAT_KP surface for
; this prompt, so kp0/kp1 stay 0 throughout.
;
; Coverage this script reaches, in order -- STATUS (M3, fully root-caused by
; live single-instruction stepping, not guessed; an earlier apparent
; "crash" chasing this turned out to be a SEPARATE test-harness artifact --
; a hand-built .cfg missing the [memattr] $8000-$9BFF RAM declaration
; `make` normally appends -- always use `make`, never a bare as1600
; invocation, when testing a new build):
;   1. settle, seat 0 picks boxer #1 and presses ENTER -- $01AC (seat 0's
;      "confirmed" flag) goes 0 -> 1; $01AE holds the chosen digit.
;   2. seat 1 picks boxer #2 (a DIFFERENT digit -- see below) and presses
;      ENTER, staggered well after seat 0's.  This DOES complete the
;      transition to BX_HTBL_LIVE.
;
;   The M3 investigation session found the real mechanism by single-
;   stepping (jzIntv's `s`, not `n` -- `n` UNSETS a breakpoint, an easy
;   trap) through $597E/$59AB-$59C5's ENTER-confirm tail with a
;   breakpoint at each conditional branch, comparing flags at $59B3 and
;   $59B7 against the two seats' live cell values:
;     $59AF-$59B3: on ENTER, cross-check the OTHER seat's "confirmed"
;       flag ($01AD-seat, i.e. $01AC+the-other-seat's-index) -- if they
;       haven't confirmed yet, this seat's own press is just a no-op
;       "mark myself confirmed" (write to $01AC+seat) and returns.
;     $59B5-$59B7: if the other seat HAS confirmed, a SECOND check
;       compares the OTHER seat's stored digit ($01AC+the-other-seat+2,
;       e.g. $01AE for seat 0) against THIS seat's own digit (R0) --
;       CMPI-equal is treated as "duplicate, ignore" and returns WITHOUT
;       transitioning.  Sending digit #1 to BOTH seats (the original
;       script) hit exactly this branch every time: two boxers can't
;       be the same pick, and the "duplicate" guard silently ate the
;       second ENTER, parking the whole run in BX_HTBL_PROMPT forever
;       with every CRC/checksum gate reporting a perfectly consistent,
;       wrong result (PORTING.md §5.5's exact trap, a new instance of it).
;     $59BA: only reached when BOTH seats have confirmed AND picked
;       DIFFERENT digits -- pushes $50A2 as a computed tail-call return
;       address; the handler's own shared $59A2-$59AA cleanup path's
;       final PULR R7 pops THAT instead of the real caller, jumping into
;       $50A2's install-the-live-table sequence.  CONFIRMED live: with
;       seat 0 -> #1 and seat 1 -> #2, $59BA/$50A2 both get hit and
;       GAME_TBL transitions to BX_HTBL_LIVE.
;   3. single-tick alternating disc PULSES, not sustained holds -- a
;      15-tick hold only produces ONE fresh edge at the prompt's disc/
;      preview handler ($5347, which reacts to fresh $011F EVENTS, not the
;      held level), so long holds are mostly flat/repeated ticks that
;      dilute the cross-correlation.  Alternating direction EVERY tick
;      makes nearly every tick a fresh, distinct edge -- then hand over
;      to the masked ($3F, disc-only) fuzz.
SCRIPT_TBL:
        DECLE   20, $40, $40, 0, 0      ; settle at boot
        DECLE   1,  $81, $40, 0, 0      ; fresh press: seat 0 picks #1
        DECLE   5,  $40, $40, 0, 0      ; release
        DECLE   1,  $8B, $40, 0, 0      ; fresh press: seat 0 ENTER
        DECLE   10, $40, $40, 0, 0      ; let it register
        DECLE   1,  $40, $82, 0, 0      ; fresh press: seat 1 picks #2
        DECLE   5,  $40, $40, 0, 0      ; release
        DECLE   1,  $40, $8B, 0, 0      ; fresh press: seat 1 ENTER
        DECLE   10, $40, $40, 0, 0      ; let the prompt->live transition settle
        DECLE   1,  $02, $02, 0, 0
        DECLE   1,  $40, $40, 0, 0
        DECLE   1,  $0C, $0C, 0, 0
        DECLE   1,  $40, $40, 0, 0
